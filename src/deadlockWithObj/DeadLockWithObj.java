...
public class DeadLockWithObj {
    private final static CountDownLatch startCDL = new CountDownLatch(1);
    ...
    Thread thread1 = new Thread("Поток 1") {
        public void run() {
            ...
            synchronized (str1) {
                synchronized (str2) {
                    System.out.printf("%s %s %s",Thread.currentThread().getName(), str1, str2);
                }
            }
        }
    };
    ...
    Thread thread2 = new Thread("Поток 2") {
        public void run() {
            ...
            synchronized (str2) {
                synchronized (str1) {
                    System.out.printf("%s %s %s",Thread.currentThread().getName(), str2, str1);
                }
            }
        }
    };
